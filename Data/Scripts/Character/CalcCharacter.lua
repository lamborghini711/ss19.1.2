...
function WizardDamageCalc(Strength, Dexterity, Vitality, Energy)
	...
	AttackDamageMax = Strength / 2 -- Maximum Left Hand Damage	
...
function KnightDamageCalc(Strength, Dexterity, Vitality, Energy, IsSpecialBuff)
	...
	if (IsSpecialBuff == 1) then -- Strong Belief
		AttackDamageMin = Strength / 5 -- Minimum Left Hand Damage
		AttackDamageMax = Strength / 3 -- Maximum Left Hand Damage		
	else
		AttackDamageMin = Strength / 3 -- Minimum Left Hand Damage
		AttackDamageMax = Strength / 2 -- Maximum Left Hand Damage		
	end
...
function ElfWithoutBowDamageCalc(Strength, Dexterity, Vitality, Energy)
	...
	AttackDamageMin = (Dexterity + Strength) / 4 -- Minimum Left Hand Damage
	AttackDamageMax = (Dexterity + Strength) / 2 -- Maximum Left Hand Damage
...
function ElfWithBowDamageCalc(Strength, Dexterity, Vitality, Energy)
	...
	AttackDamageMin = Strength / 6 + Dexterity / 4 -- Minimum Left Hand Damage
	AttackDamageMax = Strength / 3 + Dexterity / 2 -- Maximum Left Hand Damage
...
function LordDamageCalc(Strength, Dexterity, Vitality, Energy, Command)
	...
	AttackDamageMin = Strength / 4 + Energy / 7 + Command / 4 -- Minimum Left Hand Damage
	AttackDamageMax = Strength / 3 + Energy / 5 + Command / 2 -- Maximum Left Hand Damage	
...
function RageFighterDamageCalc(Strength, Dexterity, Vitality, Energy)
	...
	AttackDamageMin = Strength / 7 + Vitality / 5 -- Minimum Left Hand Damage
	AttackDamageMax = Strength / 5 + Vitality / 2 -- Maximum Left Hand Damage
...
function GrowLancerDamageCalc(Strength, Dexterity, Vitality, Energy)
	...
	AttackDamageMin = Strength / 8 + Dexterity / 5 -- Minimum Left Hand Damage	
	AttackDamageMax = Strength / 4 + Dexterity / 2 -- Maximum Left Hand Damage
...
function RuneWizardDamageCalc(Strength, Dexterity, Vitality, Energy)
	...
	AttackDamageMin = Strength / 3 -- Minimum Left Hand Damage
	AttackDamageMax = Strength / 2 -- Maximum Right Hand Damage
...
function SlayerDamageCalc(Strength, Dexterity, Vitality, Energy)
	...
	AttackDamageMin = (Dexterity / 4) + (Strength / 4) -- Minimum Left Hand Damage
	AttackDamageMax = (Dexterity / 2) + (Strength / 3) -- Maximum Left Hand Damage
...
function GunCrusherDamageCalc(Strength, Dexterity, Vitality, Energy)
	...
	AttackDamageMin = Strength / 4 -- Minimum Left Hand Damage
	AttackDamageMax = Strength / 2 -- Maximum Left Hand Damage	
...
function SummonerMagicDamageCalc(Energy, IsSpecialBuff)
	...
	if (IsSpecialBuff == 1) then -- Darkness
		...
		MagicDamageMax = Energy / 5
		CurseDamageMin = Energy / 4
		CurseDamageMax = Energy / 2
	else
		MagicDamageMin = Energy / 4
		MagicDamageMax = Energy / 2
		CurseDamageMin = Energy / 9
		CurseDamageMax = Energy / 5
	end
...
function GunCrusherMagicDamageCalc(Energy, IsSpecialBuff)
	...
	if (IsSpecialBuff == 1) then -- Fixed Fire
		MagicDamageMin = Energy / 3 -- Minimum Magic Damage
		MagicDamageMax = Energy / 2 -- Maximum Magic Damage
	else
		MagicDamageMin = Energy / 3 -- Minimum Magic Damage
		MagicDamageMax = Energy / 2 -- Maximum Magic Damage
...
function CalcAttackSuccessRate_PvM(Class, Strength, Dexterity, Vitality, Energy, Command, NormalLevel, MasterLevel)
	...
	if(Class == CLASS_WIZARD) then
		AttackSuccessRate = (TotalLevel * 5) + (Dexterity * 2.5) + (Strength / 4)
	elseif(Class == CLASS_KNIGHT) then
		AttackSuccessRate = (TotalLevel * 5) + (Dexterity * 2.5) + (Strength / 4)
	elseif(Class == CLASS_ELF) then
		AttackSuccessRate = (TotalLevel * 5) + (Dexterity * 2.5) + (Strength / 4)
	...
	elseif(Class == CLASS_SUMMONER) then
		AttackSuccessRate = (TotalLevel * 5) + (Dexterity * 2.5) + (Strength / 4)
	elseif(Class == CLASS_RAGEFIGHTER) then
		AttackSuccessRate = (TotalLevel * 5) + (Dexterity * 2.25) + (Strength / 6)
	elseif(Class == CLASS_GROWLANCER) then
		AttackSuccessRate = (TotalLevel * 5) + (Dexterity * 2.25) + (Strength / 4)
	elseif(Class == CLASS_RUNEWIZARD) then
		AttackSuccessRate = (TotalLevel * 5) + (Dexterity * 2.5) + (Strength / 4)
	elseif(Class == CLASS_SLAYER) then
		AttackSuccessRate = (TotalLevel * 5) + (Dexterity * 2.5) + (Strength / 4)
	...
	elseif(Class == CLASS_LIGHTWIZARD) then
		AttackSuccessRate = (TotalLevel * 5) + (Dexterity * 2.5) + (Strength / 4)
	elseif(Class == CLASS_LEMURIAMAGE) then
		AttackSuccessRate = (TotalLevel * 5) + (Dexterity * 2.5) + (Strength / 2)
	elseif(Class == CLASS_ILLUSIONKNIGHT) then
		AttackSuccessRate = (TotalLevel * 5) + (Dexterity * 2.5) + (Strength / 4)
	elseif(Class == CLASS_ALCHEMIST) then
		AttackSuccessRate = (TotalLevel * 5) + (Dexterity * 2.5) + (Strength / 4)
...
function CalcDefenseSuccessRate_PvM(Class, Strength, Dexterity, Vitality, Energy, NormalLevel, MasterLevel)
	...
	elseif(Class == CLASS_GLADIATOR) then
		DefenseSuccessRate = Dexterity / 2
	elseif(Class == CLASS_DARKLORD) then
...
function CalcAttackSuccessRate_PvP(Class, Strength, Dexterity, Vitality, Energy, Command, NormalLevel, MasterLevel)
	...
	if(Class == CLASS_WIZARD) then
		AttackRate = (TotalLevel * 3) + (Dexterity * 4)--Dexterity * 4 + 3 * TotalLevel
	elseif(Class == CLASS_KNIGHT) then
		AttackRate = (TotalLevel * 3) + (Dexterity * 4.5)--Dexterity * 4.5 + 3 * TotalLevel
	elseif(Class == CLASS_ELF) then
		AttackRate = (TotalLevel * 3) + (Dexterity * 0.6)--Dexterity * 0.6 + 3 * TotalLevel
	elseif(Class == CLASS_GLADIATOR) then
		AttackRate = (TotalLevel * 3) + (Dexterity * 3.5)--Dexterity * 3.5 + 3 * TotalLevel
	elseif(Class == CLASS_DARKLORD) then
		AttackRate = (TotalLevel * 3) + (Dexterity * 4)--Dexterity * 4 + 3 * TotalLevel
	elseif(Class == CLASS_SUMMONER) then
		AttackRate = (TotalLevel * 3) + (Dexterity * 3.5)--Dexterity * 3.5 + 3 * TotalLevel
	elseif(Class == CLASS_RAGEFIGHTER) then
		AttackRate = (TotalLevel * 3) + (Dexterity * 3.6)--Dexterity * 3.6 + 2.6 * TotalLevel
	elseif(Class == CLASS_GROWLANCER) then
		AttackRate = (TotalLevel * 3) + (Dexterity * 2.5)--Dexterity * 2.5 + 3 * TotalLevel
	elseif(Class == CLASS_RUNEWIZARD) then
		AttackRate = (TotalLevel * 3) + (Dexterity * 4)--Dexterity * 4 + 3 * TotalLevel
	elseif(Class == CLASS_SLAYER) then
		AttackRate = (TotalLevel * 3) + (Dexterity * 2.5)--Dexterity * 2.5 + 3 * TotalLevel
	elseif(Class == CLASS_GUNCRUSHER) then
		AttackRate = (TotalLevel * 3) + (Dexterity * 3.0)--Dexterity * 3.0 + 3 * TotalLevel
	elseif(Class == CLASS_LIGHTWIZARD) then
		AttackRate = (TotalLevel * 3) + (Dexterity * 4)--Dexterity * 4 + 3 * TotalLevel
	elseif(Class == CLASS_LEMURIAMAGE) then
		AttackRate = (TotalLevel * 3) + (Dexterity * 4)--Dexterity * 4 + 3 * TotalLevel
...
local TotalLevel = NormalLevel + MasterLevel
	...
	if(Class == CLASS_WIZARD) then
		DefenseRate = (Dexterity / 4) + (TotalLevel * 2)
	elseif(Class == CLASS_KNIGHT) then
		DefenseRate = (Dexterity / 2) + (TotalLevel * 2)
	elseif(Class == CLASS_ELF) then
		DefenseRate = (Dexterity / 10) + (TotalLevel * 2)
	elseif(Class == CLASS_GLADIATOR) then
		DefenseRate = (Dexterity / 2) + (TotalLevel * 2)
	elseif(Class == CLASS_DARKLORD) then
		DefenseRate = (Dexterity / 2) + (TotalLevel * 2)
	elseif(Class == CLASS_SUMMONER) then
		DefenseRate = (Dexterity / 2) + (TotalLevel * 2)
	elseif(Class == CLASS_RAGEFIGHTER) then
		DefenseRate = (Dexterity / 5) + (TotalLevel * 1.5)
	elseif(Class == CLASS_GROWLANCER) then
		DefenseRate = (Dexterity / 5) + (TotalLevel * 2)
	elseif(Class == CLASS_RUNEWIZARD) then
		DefenseRate = (Dexterity / 3) + (TotalLevel * 2)
	elseif(Class == CLASS_SLAYER) then
		DefenseRate = (Dexterity / 3) + (TotalLevel * 2)
	elseif(Class == CLASS_GUNCRUSHER) then
		DefenseRate = (Dexterity / 3) + (TotalLevel * 2)
...
function ElementalDefenseCalc(Class, Strength, Dexterity, Vitality, Energy, IsSpecialBuff)
	...
	elseif(Class == CLASS_DARKLORD) then
		Defense = (Strength / 5) + (Dexterity / 3)
	elseif(Class == CLASS_SUMMONER) then
		Defense = (Energy / 5) + (Dexterity / 3)
	elseif(Class == CLASS_RAGEFIGHTER) then
...
function CalcDarkHorseDefenseBonus(Dexterity, PetLevel)
 local AddDefense = Dexterity / 15 + 5 + PetLevel * 2
...
function CalcStamina(Class, Strength, Dexterity, Vitality, Energy, Leadership, NormalLevel, MasterLevel, MajesticLevel)
	...
	elseif(Class == CLASS_DARKLORD) then
		Stamina = (Strength * 0.3) + (Dexterity * 0.2) + (Vitality * 0.3) + (Energy * 0.15) + (Leadership * 0.3)
	elseif(Class == CLASS_SUMMONER) then
		Stamina = (Strength * 0.2) + (Dexterity * 0.25) + (Vitality * 0.3) + (Energy * 0.15)
	elseif(Class == CLASS_RAGEFIGHTER) then
		Stamina = (Strength * 0.15) + (Dexterity * 0.2) + (Vitality * 0.3) + (Energy * 1.0)
