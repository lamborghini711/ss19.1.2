...
function EvilSpirit_MasterLevel1_Calc(Class, InDamage, Strength, Dexterity, Vitality, Energy)
 local OutDamage = 0
 	...
 	if (Class == CLASS_WIZARD) then
		OutDamage = InDamage * (Dexterity / 700) + (Energy / 700) + 10
...
function WizardMagicDefense_Level3(Index, TargetIndex, TargetClass, Dexterity, Energy)
	...
	local SkillEffect = Dexterity / 10 + Energy / 10 + 10
	local SkillTime = Energy / 40 + 60
...
function TwistingSlashCalc_Master_Knight(InDamage, Energy)
	...
	elseif (Class == CLASS_GLADIATOR and SkillId == 481) then
		OutDamage = InDamage * (Energy / 10 + 200) / 100
	end
	return OutDamage
end
...
function GrowLancerOVersting_MasterLv1(InDamage, Dexterity, SkillTreeBonus)
...
function GrowLancerWildBreath_MasterLevel1_Calc(InDamage, Strength, SkillTreeBonus)
...
function ExpansionWizardryCalc_Level2(Class, Energy, MagicDamageMax, SkillTreeValue1,SkillTreeValue2)
	...
	if (Class == CLASS_WIZARD) then
		SkillEffect1 = (Energy / 9) * 5.20
		SkillEffect2 = (MagicDamageMax / 50.0) * SkillTreeValue1
	...
	elseif (Class == CLASS_LIGHTWIZARD) then
		...
		SkillEffect3 = SkillTreeValue2
	elseif (Class == CLASS_LEMURIAMAGE) then
		SkillEffect1 = (Energy / 2) * 10.20
		SkillEffect2 = (MagicDamageMax / 10.0) * SkillTreeValue1
		SkillEffect3 = SkillTreeValue2
	end
...
function BatFlock_MasterLevel1_DotDamage(InDamage, Strength, Dexterity)
...
function BatFlock_MasterLevel2_DotDamage(InDamage, Strength, Dexterity)
	local Damage = InDamage * ((Strength / 8) + (Dexterity / 28) + 120) / 50
	local DotDamage = (InDamage / 10)
	local Time = 13
...
function StrikeOfDestructionCalc_MasterLevel2_Knight(InDamage, Strength, Dexterity, Vitality, Energy, BarrageCount, SkillTreeValue)
	...
	elseif (BarrageCount == 2) then
		OutDamage = ((InDamage * 1.8) * (Energy / 10 + 200) + SkillTreeValue) / 100
	end
