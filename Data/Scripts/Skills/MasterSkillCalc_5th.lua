...
function MeteorStorm_5thEnchant_Wizard(InDamage,Dexterity, Energy, BarrageCount)--2hit
	
	local OutDamage = 0
	if (BarrageCount == 1) then
		OutDamage = InDamage * (Dexterity / 100) + (Energy / 50) + 10
	elseif (BarrageCount == 2) then
		OutDamage = InDamage * (Dexterity / 10) + (Energy / 50) + 10
...
function SwordBlow_5thEnchant_Knight(InDamage, Strength, Dexterity, Vitality, Energy, BarrageCount)--3hit
	...
	if (BarrageCount == 1) then
		OutDamage = (InDamage * 0.8) * (Energy / 10 + 200) / 800
	elseif (BarrageCount == 2) then
		OutDamage = (InDamage * 1.0) * (Energy / 10 + 200) / 800
	elseif (BarrageCount == 3) then
		OutDamage = (InDamage * 1.2) * (Energy / 10 + 200) / 800
	elseif (BarrageCount == 4) then
		OutDamage = (InDamage * 1.4) * (Energy / 10 + 200) / 800
	elseif (BarrageCount == 5) then
		OutDamage = (InDamage * 1.6) * (Energy / 10 + 200) / 800
	end
...
function ChaosBlade_5thEnchant_Gladiator(InDamage, Strength, Dexterity, Vitality, Energy, BarrageCount)--2hit
    local OutDamage = 0
     ...
     if (BarrageCount == 1) then
        OutDamage = (InDamage * 0.33) * ( 200 + ( Strength / 10 ) ) / 50;
    elseif (BarrageCount == 2) then
        OutDamage = (InDamage * 0.33) * ( 200 + ( Strength / 10 ) ) / 50;
    elseif (BarrageCount == 3) then
        OutDamage = (InDamage * 0.33) * ( 200 + ( Strength / 10 ) ) / 50;
...
function Oversting_5thEnchant_Lancer(InDamage, Dexterity, SkillTreeBonus)--3 hits--SkillTreeBonus is skill 712  tree value

	local OutDamage = (InDamage * 1.0) * (Dexterity / 10 + 97 + SkillTreeBonus) / 20.0
...
function WildBreath_5thEnchant_Lancer(InDamage, Strength, SkillTreeBonus)--3hit,SkillTreeBonus is 711  tree value
	
	local OutDamage = (InDamage * 0.5) * (Strength / 10 + 97 + SkillTreeBonus) / 800.0
...
function Lord_Spirit_Blast_of_Fire(InDamage, Strength, Dexterity, Vitality, Energy, Command, BarrageCount)--2hit
   ...
   if (BarrageCount == 1) then
		OutDamage = InDamage * ( 250 + ( Energy / 20 ) + ( Command / 20 ) + ( Dexterity / 20 ) ) / 50;
	elseif (BarrageCount == 2) then
		OutDamage = InDamage * ( 250 + ( Energy / 20 ) + ( Command / 20 ) + ( Dexterity / 20 ) ) / 50;
	elseif (BarrageCount == 3) then---4th tree hit
		OutDamage = InDamage * ( 250 + ( Energy / 20 ) + ( Command / 20 ) + ( Dexterity / 20 ) ) / 50;
...
function ExtraDamageCalc(InDamage, ExtraDamage, ExtraDamageType)
    ...
    --ExtraDamage:ExtraDamage value for all 5th card        
        ...
        if (ExtraDamageType == 0) then
            OutDamage = InDamage * 0.5 + ExtraDamage * 1.3--Nuke
        elseif (ExtraDamageType == 1) then
            OutDamage = InDamage * 0.5 + ExtraDamage * 1.2--Bolt
        elseif (ExtraDamageType == 2) then
            OutDamage = InDamage * 0.5 + ExtraDamage * 1.1--Aoe        
        end
        return OutDamage
end