...
function EvilSpiritCalc_4thEnchant(Class, InDamage, Strength, Dexterity, Vitality, Energy)
 local OutDamage = 0
	...
	elseif (Class == CLASS_GLADIATOR) then
		OutDamage = InDamage * (Energy / 700) + 10
	end
...
function ShiningPeak_4thEnchant_Lancer(InDamage, Strength, Dexterity, SkillTreeBonus_Retailation, SkillTreeBonus_Rage)
	...
	local OutDamage = InDamage * (((Strength / 10 + 97 + SkillTreeBonus_Retailation) + (Dexterity / 10 + 97 + SkillTreeBonus_Rage)) * 0.8) / 400.0
...
function MeteorStorm_4thEnchant_Wizard(InDamage,Energy,Dexterity,BarrageCount)
	
	local OutDamage = 0
	if (BarrageCount == 1) then
		OutDamage = InDamage * (Energy / 50) + 10
	elseif (BarrageCount == 2) then
		OutDamage = InDamage * (Energy / 50) + 10
	elseif (BarrageCount == 3) then
		OutDamage = InDamage * (Energy / 50) + 10
...
function MeteorStrike_4thEnchant_Wizard(InDamage, Energy,Dexterity, BarrageCount)
	
	local OutDamage = 0
		if (BarrageCount == 1) then
		OutDamage = InDamage * (Dexterity / 20) + (Energy / 50) + 10
	elseif (BarrageCount == 2) then
		OutDamage = InDamage * (Dexterity / 20) + (Energy / 50) + 10
	elseif (BarrageCount == 3) then
		OutDamage = InDamage * (Dexterity / 20) + (Energy / 50) + 10
...
function FireBeast_4thEnchant_Summoner(InDamage, Energy)
	local OutDamage = InDamage * (Energy/100)/100
...
function SwordBlow_4thEnchant_Knight(InDamage, Strength, Dexterity, Vitality, Energy, BarrageCount)
	...
	if (BarrageCount == 1) then
		OutDamage = (InDamage * 0.8) * (Energy / 10 + 200) / 1000
	elseif (BarrageCount == 2) then
		OutDamage = (InDamage * 1.0) * (Energy / 10 + 200) / 1000
	elseif (BarrageCount == 3) then
		OutDamage = (InDamage * 1.2) * (Energy / 10 + 200) / 1000
	elseif (BarrageCount == 4) then
		OutDamage = (InDamage * 1.4) * (Energy / 10 + 200) / 1000
	end
...
function Defense_4thEnchant_Elf(Class, Index, TargetIndex, Strength, Dexterity, Vitality, Energy)
	...
	elseif (Index == TargetIndex) then
		SkillEffect = 2 + (Energy / 2) + (Dexterity / 2)
	end
