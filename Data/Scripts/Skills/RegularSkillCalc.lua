...
function EvilSpiritCalc(Class, InDamage, Strength, Dexterity, Vitality, Energy, BarrageCount)---add
 local OutDamage = 0
 	...
 	if (Class == CLASS_WIZARD) then
		OutDamage = InDamage * (Dexterity / 100) + (Energy / 100) + 10
...
function WizardMagicDefense(Index, TargetIndex, TargetClass, Dexterity, Energy)
	...
	local SkillEffect = Dexterity / 50 + Energy / 10 + 10
	...
	elseif (Index ~= TargetIndex and SkillEffect > 50)  then -- casting spell on others
		SkillEffect = 50
	elseif (Index ~= TargetClass and SkillEffect > 50)  then -- casting spell on others
		SkillEffect = 50
...
function Wizard_NovaSkillCalc(InDamage, Energy)
 local OutDamage = InDamage * (Dexterity / 100) + (Energy / 50) + 10
 
 return OutDamage
end

-- SkillID: 42, Anger Blow+
function AngerBlow_Calc(Class, InDamage, Strength, Dexterity, Vitality, Energy)--add 42
 local OutDamage = 0
...
function WizardMeteorStrikeCalcDamage(InDamage, Energy,Dexterity, BarrageCount)---3 hits
	local OutDamage = 0
		if (BarrageCount == 1) then
		OutDamage = InDamage * (Dexterity / 100) + (Energy / 50) + 10
...
function WizardMeteorStormCalcDamage(InDamage, Energy,Dexterity, BarrageCount)---2 hits
	local OutDamage = 0
	if (BarrageCount == 1) then
		OutDamage = InDamage * (Dexterity / 100) + (Energy / 50) + 10
	elseif (BarrageCount == 2) then
...
function WizardSoulSeekerCalcDamage(InDamage,Energy)--1 hit every soul,send 3 souls use skill once
 ...
 local OutDamage = InDamage * (Energy / 50) + 10
...
function KnightFireBlowCalcDamage(InDamage, Strength, Dexterity, Vitality, Energy)---3 hits
...
function ComboSkillDamage(Strength, Dexterity, Vitality, Energy, InDamage, BarrageCount)--2hit need fix
 ...
 local ComboDamage = 0
	...
	elseif(Class == CLASS_ELF) then 
		ComboDamage = (Strength * 10.5) + Dexterity * 5 + Energy * 5 
	elseif(Class == CLASS_GLADIATOR) then
		ComboDamage = (Strength * 10.5) + Dexterity + Energy
	elseif(Class == CLASS_DARKLORD) then
		ComboDamage = (Strength * 20.5) + Dexterity * 5 + Energy * 5 
...
function GladiatorFireBloodCalcDamageAndDot(InDamage, Strength, Energy, BarrageCount)
	local OutDamage = InDamage * 2
	local SuccessRate = 6
...
function ElfAttack(Class, Index, TargetIndex, Energy,SkillEffect,SkillTime)
...
function MultiShotCalc(InDamage, Dexterity, Energy, BarrageCount)--1 hit every Arrow --add
 ...
 local OutDamage = InDamage * (Dexterity / 20 + 200) / 20
...
function ElfRainingArrow(InDamage, Strength, Dexterity, Vitality, Energy, BarrageCount)--2 hits
	...
	if (BarrageCount == 1) then
        OutDamage = InDamage * (Dexterity / 20 + 200) / 20
    elseif (BarrageCount == 2) then
        OutDamage = InDamage * (Dexterity / 20 + 200) / 20
    end
...
function ElfElementalAttack(Class, Index, TargetIndex, Strength, Dexterity, Vitality, Energy, InEffect, InTime)
	...
	SkillEffect = 0 -- result of formula 36 from FormulaData.xml
	SkillTime = InTime -- result of formula 37 from FormulaData.xml
	
	if (Index ~= TargetIndex) then
		if (Class == CLASS_WIZARD) then
			SkillEffect = 3 + Energy / 6
		elseif (Class == CLASS_KNIGHT) then
			SkillEffect = 3 + Energy / 6
		elseif (Class == CLASS_ELF) then
			SkillEffect = 3 + Energy / 6
...
function ElfElementalDefense(Class, Index, TargetIndex, Strength, Dexterity, Vitality, Energy, InEffect, InTime)
	...
	SkillEffect = 0 -- result of formula 38 from FormulaData.xml
...
function SummonerFireBeastCalcDamage(InDamage, Energy)--3 hits miss BarrageCount need fix
...
function SummonerAquaBeastCalcDamage(InDamage, Energy)--3 hits
...
function RageFighterKillingBlow(InDamage, Vitality)--4 hits
...
function RageFighterChainDrive(InDamage, Vitality)--8 hits
...
function RageFighterDarkSideIncDamage(InDamage, Vitality)--2 hits
...
function RuneWizardMagicArrowCalcDamage(InDamage, Strength, Dexterity, Vitality, Energy)--2 hits
...
function RuneWizardPlasmaBallCalcDamage(InDamage, Strength, Dexterity, Vitality, Energy)--1 hit--add
	local OutDamage = InDamage
...
function GunCrusherBurstingFlareCalc(InDamage, Strength, Dexterity, Vitality, Energy, BarrageCount,IsShockwave)--2 hits
	local OutDamage = 0
...
function GrowLancerOVersting(InDamage, Dexterity, Energy, BarrageCount, SkillTreeBonus)-----3 hits--need fix-- miss SkillTreeBonus 712
	local OutDamage = InDamage * (97 + Dexterity / 10 + SkillTreeBonus) / 20;
	if (BarrageCount == 1) then
...
function GrowLancerWildBreath(InDamage, Strength, SkillTreeBonus)-----3 hits-need fix--miss SkillTreeBonus 711
...
function LordSpiritBlast(InDamage, Strength, Dexterity, Vitality, Energy, Command, BarrageCount)-----2 hit
	...
	local OutDamage = 0
	if (BarrageCount == 1) then
		OutDamage = InDamage * ( 250 + ( Energy / 10 ) + ( Command / 10 ) + ( Dexterity / 10 ) ) / 20;
	elseif (BarrageCount == 2) then
		OutDamage = InDamage * ( 250 + ( Energy / 10 ) + ( Command / 10 ) + ( Dexterity / 10 ) ) / 20;
	end
...
function LordCrownForc(Index, TargetIndex, TargetClass, Energy, Command)
	...
	local SkillEffect = 0
	local SkillTime = 1500

		SkillEffect = Command / 10
